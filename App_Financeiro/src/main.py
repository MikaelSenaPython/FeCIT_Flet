...
def main(page: ft.Page):
    ...
    def atualizar_historico(transacoes):
        ...
        if not transacoes:
            historico_container.controls.append(ft.Text("Nenhuma transação encontrada para este filtro.", text_align=ft.TextAlign.CENTER))
        for t in transacoes:
    ...
    def adicionar_transacao(e):
        tipo = e.control.data
        if not all([txt_descricao.value, txt_valor.value, dd_categoria.value, txt_data_selecionada.value != "Selecione uma data..."]):
    ...
    card_resumo = ft.Card(elevation=5, content=ft.Container(padding=15, content=ft.Column([
        ...
        ft.Divider(),
        ft.Row([ft.Icon(ft.Icons.ACCOUNT_BALANCE_WALLET), ft.Text("Saldo:", size=18, weight=ft.FontWeight.BOLD), txt_saldo_final]),
    ])))

    # -- Componente do Cofre (Início) --
    card_cofre = ft.Card(elevation=5, content=ft.Container(padding=15, content=ft.Column([
        ft.Text("Cofre (Metas)"),
        ft.Text("R$ 0,00", size=24, weight=ft.FontWeight.BOLD),
        ft.Text("Valor Atual"),
        ft.OutlinedButton("Comece a Poupar")
    ])))
    ...
    card_resumo_dashboard_geral = ft.Card(elevation=5, content=ft.Container(padding=15, content=ft.Column([
        ...
        ft.Divider(),
        ft.Row([ft.Text("Lucro do Mês:", weight=ft.FontWeight.BOLD, size=18), txt_lucro_mes], alignment=ft.MainAxisAlignment.SPACE_BETWEEN)
    ])))
    ...
    card_resumo_dashboard_filtrado = ft.Card(elevation=5, visible=False, content=ft.Container(padding=15, content=ft.Row(
    ...
    carteira_view = ft.Column(
        [
            ...
            ft.Text("Funcionalidade para gerenciar o cofre e metas em breve!", text_align=ft.TextAlign.CENTER)
        ],
        spacing=15,
        horizontal_alignment=ft.CrossAxisAlignment.CENTER,
        visible=False
